...
export default function DashboardClient({
  ...
  const renderLostItems = (items: Item[], isMyItems: boolean) => {
    if (items.length === 0) return <p className="text-neutral-500 italic">No lost items reported yet.</p>;

    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {items.map((item) => (
          <div key={item.id} className="bg-neutral-900/50 border border-white/10 p-6 rounded-3xl hover:border-white/20 transition-colors flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <span className="bg-red-500/10 text-red-400 text-xs px-3 py-1 rounded-full font-medium">Lost</span>
              <span className="text-xs text-neutral-500">{new Date(item.created_at).toLocaleDateString()}</span>
            </div>
            <h3 className="text-lg font-medium mb-2 text-white">{item.name}</h3>
            <div className="space-y-2 text-sm text-neutral-400 mb-4 flex-1">
            ...
            <div className="p-3 bg-black/40 rounded-xl text-xs text-neutral-300 border border-white/5 mb-4">
              <span className="font-medium text-white block mb-1">Description:</span>
                ...
                {item.matches.map(match => (
                  <div key={match.id}>
                    ...
                    {match.status === 'verified' && (
                      <Link href={`/chat/${match.id}`} className="w-full flex items-center justify-center gap-2 py-2 bg-emerald-500 text-white text-sm font-medium rounded-lg hover:bg-emerald-600 transition-colors">
  ...
  const renderFoundItems = (items: Item[], isMyItems: boolean) => {
    if (items.length === 0) return <p className="text-neutral-500 italic">No found items reported yet.</p>;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {items.map((item) => (
          <div key={item.id} className="bg-neutral-900/50 border border-white/10 p-6 rounded-3xl hover:border-white/20 transition-colors flex flex-col">
            ...
            <h3 className="text-lg font-medium mb-2 text-white">{item.category} (Found)</h3>
            <div className="space-y-2 text-sm text-neutral-400 mb-4 flex-1">
              <div className="flex items-center gap-2"><Tag className="w-4 h-4" /> {item.category}</div>
            ...
            {isMyItems && item.matches && item.matches.length > 0 && (
              <div className="mt-auto space-y-2 pt-4 border-t border-white/10">
                ...
                {item.matches.map(match => (
                  <div key={match.id}>
                    {match.status === 'pending' && (
                      <p className="text-xs text-neutral-400 py-2 text-center bg-white/5 rounded-lg border border-white/5">
    ...
    <div className="space-y-12">
      <div className="flex items-center gap-4 border-b border-white/10 pb-4">
        <button 
          ...
          className={`px-4 py-2 text-sm font-medium transition-colors ${activeTab === "all" ? "text-white border-b-2 border-white" : "text-neutral-500 hover:text-neutral-300"}`}
        >
          All Items
        </button>
        {user && (
          <button 
            ...
            className={`px-4 py-2 text-sm font-medium transition-colors ${activeTab === "my" ? "text-white border-b-2 border-white" : "text-neutral-500 hover:text-neutral-300"}`}
          >
            My Items
          </button>
        )}
      ...
      {activeTab === "all" && (
        <div className="space-y-16 animate-in fade-in duration-500">
          <div className="space-y-6">
            <h2 className="text-2xl font-medium tracking-tight text-white border-b border-white/10 pb-4">All Lost Items</h2>
            {renderLostItems(lostItems, false)}
          </div>
          <div className="space-y-6">
            <h2 className="text-2xl font-medium tracking-tight text-white border-b border-white/10 pb-4">All Found Items</h2>
      ...
      {activeTab === "my" && user && (
        <div className="space-y-16 animate-in fade-in duration-500">
          <div className="space-y-6">
            <h2 className="text-2xl font-medium tracking-tight text-white border-b border-white/10 pb-4">My Lost Items</h2>
            {renderLostItems(myLostItems, true)}
          </div>
          <div className="space-y-6">
            <h2 className="text-2xl font-medium tracking-tight text-white border-b border-white/10 pb-4">My Found Items</h2>
